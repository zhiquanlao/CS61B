...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        while (index < size
        && (min(index, leftIndex(index)) == leftIndex(index) || min(index, rightIndex(index)) == rightIndex(index))) {
            int newIndex = min(leftIndex(index), rightIndex(index));
            swap(index, newIndex);
            index = newIndex;
        }
        return;
    ...
    public static void main(String[] args) {
        jh61b.junit.TestRunner.runTests(ArrayHeap.class);
    }
}
